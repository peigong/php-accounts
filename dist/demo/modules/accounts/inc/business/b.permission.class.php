 ...
 * 账户系统业务层许可类。
 */
class BPermission extends BObject implements IBPermission{  
    /*- IBPermission 接口实现 START -*/
    ...
    public function fetchModelList($code, $ext){
        ...
        foreach ($rows as $idx => $row) {
            $entity = $this->manager->getAttributeValues(MODEL_TYPE_PERMISSION, $row['id'], $attributes);
            array_push($result, $entity);
        }
        return $result;
    }
    /*- IModelListFetch 接口实现 END -*/
    /*- IBPermission 接口实现 END -*/
